...
import { AppStoreError, ensureAppSchema, getSql, isRecord, normalizeJsonRecord, toIsoString } from "./db";
...
export type InterviewBundle = InterviewRecord & { assets: InterviewAsset[]; messages: ConversationMessage[] };
export type CreateInterviewInput = Record<string, unknown>;
...
export async function listInterviews(userLogin: string): Promise<InterviewRecord[]> {
  ...
  const rows = await getSql()<InterviewRow[]>`select * from interviews where user_login = ${userLogin} order by updated_at desc`;
...
export async function createInterview(userLogin: string, input: CreateInterviewInput): Promise<InterviewRecord> {
  ...
  const taskMemory = { company, position, targetLevel, interviewType, sourceType, sourceUrl };
  const [row] = await getSql()<InterviewRow[]>`
    insert into interviews (id, user_login, company, position, target_level, interview_type, source_type, source_url, task_memory)
    values (${id}, ${userLogin}, ${company}, ${position}, ${targetLevel}, ${interviewType}, ${sourceType}, ${sourceUrl}, ${getSql().json(taskMemory)})
    returning *
  `;
  if (!row) throw new AppStoreError("Interview was not created.");
...
export async function getInterviewBundle(userLogin: string, id: string): Promise<InterviewBundle | null> {
  ...
  return { ...interview, assets, messages };
}

export async function getInterview(userLogin: string, id: string): Promise<InterviewRecord | null> {
  await ensureAppSchema();
  const [row] = await getSql()<InterviewRow[]>`select * from interviews where user_login = ${userLogin} and id = ${id} limit 1`;
  return row ? rowToInterview(row) : null;
}

export async function listAssets(interviewId: string): Promise<InterviewAsset[]> {
  await ensureAppSchema();
  const rows = await getSql()<AssetRow[]>`select * from interview_assets where interview_id = ${interviewId} order by created_at asc`;
  return rows.map(rowToAsset);
}

export async function createAsset(interviewId: string, input: AssetInput): Promise<InterviewAsset> {
  await ensureAppSchema();
  if (input.kind === "recording" && Number(input.sizeBytes ?? 0) > MAX_RECORDING_BYTES) throw new AppStoreError("Recording is larger than 2 GB.", 400);
  if (input.kind === "screenshot" && (await listAssets(interviewId)).filter((asset) => asset.kind === "screenshot").length >= MAX_SCREENSHOTS) {
    throw new AppStoreError("You can attach up to 10 screenshots.", 400);
  }
  const id = randomUUID();
  const [row] = await getSql()<AssetRow[]>`
    insert into interview_assets (id, interview_id, kind, source_type, original_name, content_type, size_bytes, s3_key, external_url, upload_status, provider_metadata)
    values (${id}, ${interviewId}, ${input.kind}, ${input.sourceType}, ${input.originalName ?? ""}, ${input.contentType ?? ""}, ${input.sizeBytes ?? 0}, ${input.s3Key ?? ""}, ${input.externalUrl ?? ""}, ${input.uploadStatus ?? "pending"}, ${getSql().json(input.providerMetadata ?? {})})
    returning *
  `;
  if (!row) throw new AppStoreError("Asset was not created.");
...
export async function markAssetUploaded(userLogin: string, interviewId: string, assetId: string): Promise<InterviewAsset | null> {
  ...
  const [row] = await getSql()<AssetRow[]>`
    update interview_assets set upload_status = 'uploaded', updated_at = now()
    where id = ${assetId} and interview_id = ${interviewId} returning *
  `;
  return row ? rowToAsset(row) : null;
}

export async function updateInterviewPatch(userLogin: string, id: string, patch: Partial<{
  status: InterviewStatus;
  taskMemory: Record<string, unknown>;
  llmOutput: Record<string, unknown>;
  providerMetadata: Record<string, unknown>;
  ...
  const [row] = await getSql()<InterviewRow[]>`
    update interviews set status = ${next.status}, task_memory = ${getSql().json(next.taskMemory)}, llm_output = ${getSql().json(next.llmOutput)},
      provider_metadata = ${getSql().json(next.providerMetadata)}, transcript = ${next.transcript}, transcript_metadata = ${getSql().json(next.transcriptMetadata)},
      error_message = ${next.errorMessage}, updated_at = now()
    where user_login = ${userLogin} and id = ${id} returning *
  `;
  return row ? rowToInterview(row) : null;
}

export async function appendConversationMessage(userLogin: string, interviewId: string, role: "user" | "assistant", content: string, providerMetadata: Record<string, unknown> = {}): Promise<ConversationMessage> {
  await ensureAppSchema();
  await assertInterviewAccess(userLogin, interviewId);
  const id = randomUUID();
  const [row] = await getSql()<MessageRow[]>`
    ...
    values (${id}, ${interviewId}, ${userLogin}, ${role}, ${content}, 'working', ${getSql().json(providerMetadata)}) returning *
  `;
  if (!row) throw new AppStoreError("Message was not saved.");
...
export async function listConversationMessages(userLogin: string, interviewId: string, limit = 200): Promise<ConversationMessage[]> {
  ...
  const rows = await getSql()<MessageRow[]>`
    select * from conversation_messages where user_login = ${userLogin} and interview_id = ${interviewId} order by created_at asc limit ${limit}
  `;
...
export async function saveLlmRun(input: {
  ...
  await ensureAppSchema();
  await getSql()`
    insert into llm_runs (id, interview_id, user_login, run_type, provider, model, prompt_variables, input_messages, output, provider_metadata)
    values (${randomUUID()}, ${input.interviewId ?? null}, ${input.userLogin}, ${input.runType}, ${input.provider}, ${input.model}, ${getSql().json(input.promptVariables)}, ${getSql().json(input.inputMessages)}, ${getSql().json(input.output)}, ${getSql().json(input.providerMetadata)})
  `;
}

export async function recordAudit(userLogin: string, interviewId: string | null, layer: string, action: string, payload: Record<string, unknown>) {
  await ensureAppSchema();
  await getSql()`insert into memory_audit_events (id, user_login, interview_id, layer, action, payload)
    values (${randomUUID()}, ${userLogin}, ${interviewId}, ${layer}, ${action}, ${getSql().json(payload)})`;
}
